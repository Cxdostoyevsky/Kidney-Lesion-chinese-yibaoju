            ...
            cases.append(
                {
                    "case_id": case_id,
                    "image": [
                        str(arterial.relative_to(data_root)),
                        str(plain.relative_to(data_root)),
                    ],
                    "label": str(label_path.relative_to(data_root)),
...
def write_datalist(cases: list[dict], data_root: Path, work_dir: Path, folds: int, seed: int) -> Path:
...
def main() -> None:
    ...
    datalist_path = write_datalist(
        folded_cases, args.data_root, args.work_dir, args.folds, args.seed
    )
