...
WORK_DIR="${WORK_DIR:-${AUTOSEG_ROOT}/segresnet3D_5fold_原始HU值}"
PYTHON="${PYTHON:-/ssd/chenxi/anaconda3/envs/nnunet/bin/python}"
TORCHRUN="${TORCHRUN:-/ssd/chenxi/anaconda3/envs/nnunet/bin/torchrun}"
PREPARE_SCRIPT="${PREPARE_SCRIPT:-${AUTOSEG_ROOT}/prepare_segresnet_original_hu_5fold.py}"
MASTER_PORT_BASE="${MASTER_PORT_BASE:-29641}"
TMPDIR="${SEGRESNET_ORIGINAL_HU_TMPDIR:-/dev/shm/user-segresnet-original-hu-tmp}"
...
bundles_ready() {
  ...
  for fold in 0 1 2 3 4; do
    [[ -f "${WORK_DIR}/segresnet_${fold}/configs/hyper_parameters.yaml" ]] || return 1
    [[ -f "${WORK_DIR}/segresnet_${fold}/scripts/train.py" ]] || return 1
  done
  return 0
}

prepare_if_needed() {
  if bundles_ready; then
    echo "[$(date '+%F %T')] SegResNet bundles already exist; skipping Auto3DSeg generation."
    return 0
  fi

  echo "[$(date '+%F %T')] Preparing datalist, input YAML, datastats, and SegResNet bundles."
...
echo "[$(date '+%F %T')] Starting original-HU SegResNet 5-fold workflow."
...
echo "[$(date '+%F %T')] All five original-HU folds completed successfully."
